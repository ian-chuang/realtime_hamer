...
class HandEstimate:
    ...
    """(778, 3) mesh at origin (wrist-centered, identity global orient), or None."""
...
class HandPoseEstimator:
    """Detect one hand (RTMPose+TRT EP) and reconstruct with HaMeR TensorRT."""
    ...
    def __init__(
        ...
        # MANO stays in PyTorch; neural net runs as a TRT engine.
        ...
        self._detector = create_detector(hand=hand, device="cuda", mode="lightweight")
    ...
    def _reconstruct(self, frame_bgr: np.ndarray, hand: HandDet) -> np.ndarray:
        ...
        img = batch["img"]  # (1,3,256,256)

        outs = self._trt({"img": img})
        # Fixed-root: identity global orient, use predicted finger pose + shape.
